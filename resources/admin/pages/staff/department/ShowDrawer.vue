<template>
    <el-drawer v-model="is.show" title="部门详情" size="560">
        <el-skeleton :rows="10" :loading="is.loading"/>
        <el-descriptions :column="1" label-align="right" label-size="small" border v-if="!is.loading">
            <el-descriptions-item label="ID"> {{ item.id }}</el-descriptions-item>
            <el-descriptions-item label="上级部门">
                {{ item.parent ? item.parent.title : '-' }}
            </el-descriptions-item>
            <el-descriptions-item label="部门名称"> {{ item.title }}</el-descriptions-item>
            <el-descriptions-item label="部门简介"> {{ item.brief ?? '-' }}</el-descriptions-item>
            <el-descriptions-item label="显示排序"> {{ item.asc_num }}</el-descriptions-item>
            <el-descriptions-item label="人员数量"> {{ item.staff_count }}</el-descriptions-item>
            <el-descriptions-item label="创建时间"> {{ utcDateTimeToLocal(item.created_at) }}</el-descriptions-item>
            <el-descriptions-item label="更新时间"> {{ utcDateTimeToLocal(item.updated_at) }}</el-descriptions-item>
        </el-descriptions>
        <template #footer>
<!--            <el-button @click="is.show=false">取消</el-button>-->
            <el-button type="primary" @click="() => { $parent.$refs.formRef.edit(item.id); is.show=false}">编辑
            </el-button>
        </template>
    </el-drawer>

</template>

<script setup>
import { api } from '../../../stores/staffDepartment';
import { ShowActions } from '../../../lib';
import { utcDateTimeToLocal } from '../../../lib/util';

const actions = new ShowActions(api);
const {
    is,
    item,
    show
} = actions;

defineExpose({
    show: (id) => show(id, { with_parent: 1 })
});
</script>
