    ...
    <el-form ref="formRef" :model="item" label-width="100px" class="show-wp">
        <el-drawer v-model="is.show" title="商品详情" size="720">
            ...
            <div v-if="!is.loading">
                <div>
                    ...
                    <el-descriptions title="基本信息" :column="1" label-align="right" label-size="small" border>
                        ...
                        <el-descriptions-item label="更新时间">{{
                                utcDateTimeToLocal(item.updated_at)
                            }}
                        </el-descriptions-item>
                        <el-descriptions-item label="删除时间">
                            {{ item.deleted_at ? utcDateTimeToLocal(item.deleted_at) : '-' }}
                        </el-descriptions-item>
                    </el-descriptions>
                </div>

                <el-descriptions class="mt-6" title="商品规格" :column="1" label-align="right" label-size="small"
                                 border>
                    <el-descriptions-item label="规格选项">
                        <span v-if="!item.has_sku">无</span>
                ...
                <el-descriptions class="mt-6" :column="1" title="商品介绍" label-align="right" label-size="small"
                                 border>
                    <el-descriptions-item label="商品简介">{{ item.brief ?? '-' }}</el-descriptions-item>
            ...
            <template #footer>
                <el-button type="primary" @click="() => showBindings.toEdit($parent.$refs.formRef, item.id)">编辑</el-button>
            </template>
        </el-drawer>
    </el-form>
</template>
...
import { api } from '../../stores/goods';
import { ShowActions } from '../../lib';
import { refKit, utcDateTimeToLocal } from '../../lib/util';
